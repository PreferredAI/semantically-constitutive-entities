...
class Tester(object):
    ...
    def run_link_prediction_maxrank(self, type_constrain = False):
        """
        This function uses testHeadMaxRank and testTailMaxRank instead, which ranks ties in a manner similar to
        scipy.stats.rankdata with method="max" (i.e., maximum rank for all tied items).
        """
        self.lib.initTest()
        self.data_loader.set_sampling_mode('link')
        if type_constrain:
        ...
        for index, [data_head, data_tail] in enumerate(training_range):
            score = self.test_one_step(data_head)
            self.lib.testHeadMaxRank(hex(score.__array_interface__["data"][0]).encode(), index, type_constrain)
            score = self.test_one_step(data_tail)
            self.lib.testTailMaxRank(hex(score.__array_interface__["data"][0]).encode(), index, type_constrain)
        self.lib.test_link_prediction(type_constrain)
